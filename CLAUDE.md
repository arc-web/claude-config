...
- **Plane tasks: all fields + handoff prompt + attribution.** Every task must have all fields populated (name, state, description, module, time estimate). Description must include a handoff prompt (context / repo / last state / next action / run command) so any agent or human can pick up mid-task. Every comment or update must end with attribution: `— [Agent: claude-sonnet-4-6 via Claude Code | YYYY-MM-DD]` or `— [Human: name | YYYY-MM-DD]`. If a task is missing its handoff prompt, analyze and add it. See `feedback_plane_task_fields.md`.
