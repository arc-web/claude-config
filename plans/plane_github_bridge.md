...
### 1. Webhook receiver on zeroclaw (replaces GitHub Secrets approach)

GitHub Actions cannot reach OpenBao directly (it's on VPS at 127.0.0.1). Copying the Plane API key to GitHub Secrets violates the credential architecture - credential values must stay in OpenBao and never travel.

**Correct pattern:**
- zeroclaw runs a webhook listener (small HTTP server, similar to `approval-webhook` already running)
- GitHub sends a signed POST (HMAC-SHA256 with a webhook secret) to zeroclaw on PR events
- zeroclaw verifies the signature, then calls Plane API using its locally-available OpenBao creds
- The only thing in GitHub is a webhook HMAC signing key (not an API key - a transport secret)

The existing `approval-webhook` service on zeroclaw proves this pattern is already in use. This is the same model.

**What the webhook listener does:**
- Verify GitHub HMAC signature (reject unauthenticated events)
- Parse PR body for `[TASK-ID - title](url)` pattern
- Fetch Plane API key from local OpenBao
- Call Plane API to update task state
- Return 200

**What GitHub has:**
- One webhook secret (HMAC signing key) per repo or org - stored as GitHub webhook secret (not Actions secret). This is a signature verifier, not a credential.
